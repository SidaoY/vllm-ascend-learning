...
Hybrid KV cache 是 vLLM 社区引入的一种混合管理策略，用于处理模型中同时存在多种 attention 类型的情况。vLLM 中所有 attention 类型由 `KVCacheSpec` 子类定义（详见 `$PATH_TO_VLLM/vllm/v1/kv_cache_interface.py`）：

| Spec 类型 | 用途 | 典型模型 |
| --- | --- | --- |
| `FullAttentionSpec` | 标准 full attention | LLaMA、Qwen、Mistral 等 |
| `MLAAttentionSpec` | Multi-head Latent Attention | DeepSeek-V2/V3/V3.2/V4 |
| `SlidingWindowSpec` | 滑动窗口 attention | Gemma 2/3、Mistral |
| `SlidingWindowMLASpec` | 滑动窗口 + MLA 格式 | — |
| `ChunkedLocalAttentionSpec` | 分块局部 attention | Llama 4 |
| `MambaSpec` | Mamba 状态空间模型 | Jamba、Bamba、Minimax-M1 |
| `CrossAttentionSpec` | 交叉注意力 | 编码器-解码器模型 |
| `SinkFullAttentionSpec` | 带 sink token 的 full attention | — |
| `TQFullAttentionSpec` | TurboQuant 感知的 full attention | — |
...
| --- | --- | --- |
| Full attention | 需要完整历史 KV | 按完整上下文分配 block |
| Sliding window attention | 只需要最近 W 个 token 的 KV | 按窗口大小分配 block |
| Mamba | 状态空间卷积缓存 | 按固定大小分配 |

如果统一按 full attention 分配，SWA 层会浪费大量显存。Hybrid KV cache manager 为不同类型的 attention 层分别管理 block pool，让 SWA 层只分配窗口大小的 block。

### 哪些模型真正使用 Hybrid KV Cache

Hybrid KV cache 的分组逻辑在 `$PATH_TO_VLLM/vllm/v1/core/kv_cache_utils.py` 的 `get_kv_cache_groups` 函数中，按优先级依次判断：

1. **所有层 spec 完全相同** → 统一分组（大多数模型，如 LLaMA、Qwen、DeepSeek-V2/V3）
2. **所有层需要相同 token slot 数**（如同为 full attention 但 head_size 不同）→ `UniformTypeKVCacheSpecs` 统一分组
3. **DeepSeek-V4 特例**：所有层 token slot 数相同，但部分层是 full attention、部分层是 sliding window attention → 分组为多个 `UniformTypeKVCacheSpecs`
4. **真正的 hybrid**：层之间 token slot 需求不同（如 full + SWA）→ 统一 page size 后分组

真正触发第 4 步 hybrid 分组的模型包括：

| 模型 | Attention 组合 |
| --- | --- |
| Gemma 2 / Gemma 3 | Full attention + Sliding window attention |
| Mistral (部分版本) | Full attention + Sliding window attention |
| Cohere Command-R | Full attention + Sliding window attention |
| GPT-OSS | Full attention + Sliding window attention |
| Bamba / Jamba | Full attention + Mamba |
| Minimax-M1 | Full attention + Mamba |
| Llama 4 | Full attention + Chunked local attention |

**DeepSeek-V2/V3 不使用 hybrid KV cache**：它们所有层都是 MLA，spec 完全相同，走第 1 步统一分组。

**DeepSeek-V3.2 的 Indexer**：V3.2 在 MLA 基础上增加了 `Indexer` 模块（由 `config.index_topk` 标识），用于稀疏 attention 的 top-k token 选择。Indexer 有自己独立的 `k_cache`（`DeepseekV32IndexerCache`，使用 `MLAAttentionSpec(num_kv_heads=1)`），但它是 MLA attention 内部的一个子组件，不作为独立的 KV cache group 参与 hybrid 管理。Indexer 的 KV cache 与 MLA 的 KV cache 在同一个 layer 内共存，由 `UniformTypeKVCacheSpecs` 统一管理。
