...
要知道 vllm-ascend **不是一个 fork**——`vllm serve` 启动的仍然是 vLLM 本身。vllm-ascend 是一个 **OOT（Out-Of-Tree）插件**，通过三种机制把自己"注入"到 vLLM 框架中：

### 1. 平台注册（Platform Registration）

vLLM 启动时通过 `NPUPlatform` 发现当前运行环境是 Ascend NPU。这个类继承 vLLM 的 `Platform` 基类，声明 `_enum = PlatformEnum.OOT`，告诉 vLLM："我是外部插件，设备类型是 npu，请用我的实现"。

```
vllm serve 启动
  → NPUPlatform 被识别为当前平台
  → 所有平台相关的扩展点（Worker、Attention Backend、量化方法...）切换到 Ascend 实现
```

### 2. 组件替换（Component Injection）

平台一旦注册，以下组件会自动切换到 Ascend 版本：

| 替换点 | 上游 vLLM | vllm-ascend 替换 |
|---|---|---|
| Worker | GPUWorker | `NPUWorker` |
| Model Runner | GPUModelRunner | `NPUModelRunner`（继承自 GPUModelRunner） |
| Input Batch | InputBatch | `NPUInputBatch` |
| Attention Backend | FlashInfer / FlashAttention | `AscendSFA/DSA/MLA/FA3/CP` |
| 通信后端 | NCCL | `HCCL` |
| KV 传输 | 无/P2P | `Mooncake / Ascend Store / LMCache / UCM` |

这些都是**继承和实现**，不是暴力替换——vLLM 框架自身的调度、Engine、API 层完全不受影响。

### 3. 必要补丁（Patch）

少数情况下，上游 vLLM 还没有提供干净的扩展点（比如某个配置校验函数硬编码了 CUDA 行为），vllm-ascend 会通过 **Monkey-patch** 临时修正。每个补丁都有明确的退出计划——等上游 vLLM 合入对应 PR 后就删除。详见 [02-patch-mechanism.md](02-patch-mechanism.md)。

### 一句话

> **跑的还是 vLLM，只是把"发动机"从 CUDA 换成了 Ascend NPU。** 方向盘、车架、调度系统还是原来的。
...
| Attention | backend 抽象 | Ascend dense attention、MLA、SFA、DSA、FA3、CP attention |
